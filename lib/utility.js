...
exports.resolve = function(mobilize, path) {
    ...
    return url.resolve(uriParts.protocol + '//' + uriParts.hostname + (uriParts.port ? ':' + uriParts.port : '') + (uriParts.pathname || ''), path);
};
...
var retrieveRemoteResource = exports.retrieveRemoteResource = function download(target, encoding, callback) {
    ...
    ).on('error', function() { callback(new Error("Failed to download " + target)); });
};
...
exports.renderDocument = function(document) {
    ...
    return document.innerHTML.replace('<html', (document.doctype || '<!DOCTYPE html>') + ' <html');
};
