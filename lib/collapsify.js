...
var collapsify = exports.collapsify = function( resourceRoot, options ) {
	...
	var read = function( resourceURL ) {
			...
			console.log(resourceURL, re.test(resourceURL));
		...
		flattenStylesheet = function( rawCSS, resourceLocation, imported ) {
			...
			parser.addListener( 'startmedia', function( event ) {
				...
				compilationQueue = compilationQueue.then( function( stylesheet ) {
					...
					return stylesheet + '@media' + event.media.reduce( function( previous, media ) {return (previous ? previous + ',' : '') + ( media.modifier ? ' ' + media.modifier : '' ) + ( media.mediaType ? ' ' + media.mediaType.text : '' ) + ( media.mediaType && media.features ? ' and ' : '' ) + media.features.reduce( function( features, feature ) {return (features ? features + ' and ' : '') + feature.text;}, '' );}, '' ) + '{';
				} );
			} );
			...
			parser.addListener( 'startrule', function( event ) {
				...
				compilationQueue = compilationQueue.then( function( stylesheet ) {
					...
					return stylesheet + event.selectors.map( function( selector ) {return selector.text.replace( '   ', ' ' );} ).join( ',' ) + '{';
				} );
			} );
