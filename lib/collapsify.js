...
module.exports = function (resourceRoot, options) {
    ...
    return Bluebird.try(function () {
      ...
      tree.match({tag: 'script'}, function (node) {
        ...
        if (node.attrs && node.attrs.type && node.attrs.type !== 'text/javascript' && node.attrs.type !== 'text/javascript') {
          options.logger.debug('ignoring <script type="%s">', node.attrs.type);
          return node;
        }
  ...
  function flattenHTML(rawHTML) {
    ...
    return lazy
      .then(function (result) {
        options.logger.trace('posthtml: %s', result.html);
        return result.html;
      })
      .catch(function (err) {
        options.logger.error('posthtml error:', err);
        return '';
      });
  }
  ...
  function flattenJavaScript(rawJavaScript) {
    return Bluebird.try(function () {
      ...
      options.logger.error('Failed to minify some JavaScript: ', err);
      return '';
    });
  }
  ...
  function flattenStylesheet(rawCSS, resourceLocation, imported) {
    ...
    return lazy
      .then(function (result) {
        ...
        options.logger.trace('postcss: %s', result.css);
